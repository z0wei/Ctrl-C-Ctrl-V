...
Если сервер обрезает длинный запрос (как в твоей лаборатории), воспользуйся следующими подходами:
...
| ⚖️ Конкатенация с `||` длиннее `AND` | Используй `AND`, если это короче. |
